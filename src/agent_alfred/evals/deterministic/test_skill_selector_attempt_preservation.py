...
from agent_alfred.evals.deterministic.test_runtime_skills import runtime, skill
...
def test_ce17_ce21_selector_retry_and_fallback_preserve_actual_attempts(
    strategy, tmp_path
):
    clock = FakeClock()
    sent, hooks = [], []
    ...
    with httpx2.Client(
        ...
        settings = Settings(overall_deadline_s=10, max_steps=2)
        skill(tmp_path / "host/builtin", "A", "procedure")
        ...
        with runtime(
            ...
            if strategy in ("fallback", "retry", "nested"):
                assert result.memory_telemetry["skills"]["reason"] == "model_deadline"
            inputs = result.memory_telemetry["input_attempts"]
            assert [item["purpose"] for item in inputs] == (
                ["skill_selector"] * gate_count + ([] if interrupted else ["answer"])
            )
