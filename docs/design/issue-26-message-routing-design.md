...
revision: ROUTING-DESIGN-r5 | 2026-09-14 | CONFIRMED；D4 整体确认；READY FOR TO-SPEC

本文件是 grill-with-docs 的设计与验收来源。只进行设计，不开始产品实现、运行验收或远端发布。未确认建议不能作为实施合同。

## 整体批准记录与冻结来源

D4：2026-09-14T13:29:37+08:00，用户在本会话对完整 ROUTING-DESIGN-r4 回复“确认”。被确认文件 SHA-256 为 `bc1afd6a76e82f6349cd20ee2c80967000021ce8c402ea028a6a8a7b57bbb448`；覆盖 R01–R08、Q1–Q16、F01–F07、CE-01–CE-14、CONTEXT 相关术语、ADR-0036/0037、公开验证接缝、外部替换及排除项。

本修订 r5 仅登记批准与交接状态，不改变 r4 的行为或验收合同。批准来源原始字节及文档附件哈希保存于本工作树 `tmp/agent-work/issue-26/design-approval/ROUTING-DESIGN-r4.md` 和 `approval.json`；它们是冻结证据，当前状态仍只由同一 LOOP 维护。

未决设计选择及验收合同阻塞：none。产品实现、运行验收及远端发布未开始；D4 不授予实现/提交/推送/PR/合并/关票权限。下一阶段为用户调用 to-spec 后，以本文件生成权威实施规范，保留全部 ID 与已确认上游补充。

## 来源与当前事实

- [#26](https://github.com/nineofoursyrup/Agent-Alfred/issues/26)：本次回读 OPEN，无评论。要求默认关闭、分类与上下文读取汇合、快速/完整/无动作出口、未知分类显式 fallback、上下文失败 error 恢复、禁止有副作用后自动重跑。
- [#6 裁决](https://github.com/nineofoursyrup/Agent-Alfred/issues/6#issuecomment-5429217158) §1–§16，以及 [GRAPH-SPEC-r1](issue-25-graph-spec.md)、[SKILL-SPEC-r1](issue-24-skill-spec.md) 是已定约束。#25 本次远端回读 CLOSED；#24 合并已存在于当前基线。本次未重新验证它们的 CI。
- 实际设计工作树 `/Users/nineofour/Agent-Alfred-issue-26-design`，分支 `codex/26-message-routing-design`；base/HEAD 为远端 main `8ba7192ee84533aef3deb8453d8fa7364d67f48b`。原主工作区 main `31daa31` 已陈旧，其 `.gitignore` 用户改动保留。
- 唯一任务入口：`tmp/agent-work/issue-26/LOOP.md`。进入时该票无已有工作树、状态入口或设计文件。
- 当前已具备生产 Graph 引擎和 `runtime/chat_graph.py::run_chat_graph` 执行适配，Host 提供 `chat_graph_factory` 注入。生产 wiring 未安装本票业务图；尚无 Behaviour 业务设置。
- `runtime/execution.py` 在图前选择一次 Skill；`runtime/chat_graph.py` 在图前调用 `RunMemory.prepare/evaluate`。因此不能仅靠新增一个图内分类器就宣称减少原有输入准备开销。
- 持久模型指派只有 `primary` / `retrieval_gate`（`runtime/model_settings.py`）。D1/Q5 已确认本票复用辅助模型位置，尚未实现。
- `NoAction` 现有 Host 适配设置 `record_reply=False`；`RunRecorder._finalize` 仍保存用户消息，只省略助手消息。CLI 对 `reply is None` 不调用回复渲染器。是否要写长期记忆是另一项业务动作，不能由这个标记证明。
- `graph/nodes.py` / `engine.py` 会保存并追加成功模型节点的运行转录；本票分类数据与回答输入之间需要明确隔离合同，不能只验证 agent_log 没有分类文本。

- `runtime/chat_graph.py` 尚未把 `NoAction.reason_code/recoveries` 写入 Run telemetry；失败回退仅标记 fallback，没有 emit Notice。`events.py` 已有持久 Notice 家族，但 code 闭合集需随本票设计扩充。
- Web 的 `ops/static/app.js` 对无 reply 的结束事件使用“运行已结束”，正文恢复分支要求字符串；`runs.js` 把 completed 称为“回复完成”。这是源码可见的适配风险，尚未通过浏览器复现，不冒称已观察到页面故障。
- 公开用户路径为 `POST /api/sessions` → `POST /api/runs` → Host → Graph/Assistant/Registry → recorder/SQLite，观察 `GET /api/events`、`/api/sessions/messages`、`/api/runs`、`/api/reply` 与浏览器。Web 提交异步且必须显式 session_id；单独 Host.wait 测试不能替代这条链。
...
D1：2026-09-14，用户对首轮 Q1–Q5 回复“全按建议”。批准的是本轮建议，不是整体设计、实现或发布授权。以下 Q1–Q5 为 confirmed。
...
D2：2026-09-14，用户对 Q6–Q12 回复“全按建议”。本轮行为与验收边界 confirmed；未授予整体设计确认、实现或发布权限。
...
D3：2026-09-14，用户对 Q13–Q16 回复“全按建议”。确认上下文失败后禁止绕回、不回复续轮规则、配置恢复及受控发布新冻结图的有限上游补充。后续整体设计已由 D4 确认；没有实现或远端动作授权。
...
下面的稳定 ID 为后续 source acceptance inventory。继承条款及 D1–D3 的 Q1–Q16 已确定。各 CE 行为已按相应批准来源收敛，F01–F07 的精确清单/字段/接缝已由整体确认 D4 覆盖。当前全部运行证据为 NOT RUN。
...
- Q1–Q16 已由 D1–D3 逐轮确认；没有需要再次选择的产品分支。F01–F07 是这些决定的完整、可复核落点，已与 R01–R08 / CE-01–CE-14 / 两项 ADR 一同获整体确认 D4。
- 状态为 CONFIRMED / READY FOR TO-SPEC；本设计是 to-spec 的冻结 source acceptance inventory。不能在 to-spec 中重新访谈已定选择或丢弃 CE；实施规范的 ready-for-agent 状态由 to-spec 后续独立记录，设计确认不自动开始实现。
- 产品代码未修改，模型/工具/浏览器/CI 全部 NOT RUN；双轴 NOT REVIEWED。设计一致性检查不是产品通过证据。
- 排除：长期保存并静默的专用组合能力、开放式快速短答、通用流程编辑器/完整 Graph 页面、#27 手动聚合、变长工作记忆组、默认付费语义评测、每 Run 重编译、wildcard 工具权限。排除不意味着旁路现有普通循环已有能力。
- 同一入口 `tmp/agent-work/issue-26/LOOP.md` 维护当前阶段与批准；不复制设计内容到状态日记。上游 #6/#25 的有限修订仅落本地设计与 ADR，未授权发布 GitHub 回写。

## 第二轮与验收清单的对应

| 决定 | 必须补齐或确认的 CE |
|---|---|
| Q6 | CE-02/03/04；整条匹配、混合任务、引用与 Skill 控制；清单外保守完整循环 |
| Q7 | CE-05；明确纯投影错误与真实数据库/遗忘安全故障不可混淆；正常与恢复 state 所有权 |
| Q8 | CE-08；分类请求输入、分类转录隔离、回答发送前真实来源核验 |
| Q9 | CE-04/06/10；局部/总体期限、辅助模型不可用、剩余预算、故障后只回退一次 |
| Q10 | CE-07；在 accepted/running/recording_pending 各时点修改、旧 revision、外部编辑、损坏/新 schema 与重启 |
| Q11 | CE-06/09/10；NoAction/恢复/回退的在线显示、重连/重启/trace 裁剪后的持久恢复，trace 与 recording 失败区分 |
| Q12 | CE-01 以及全票真实 CLI/HTTP/SSE/browser 验收边界 |

D2 已确认上表行为与验收方向；D3 补齐 CE-11–CE-14。精确规则见以下整合合同，不因确认设计声称产品测试通过。

## 整合合同 F01–F07（ROUTING-DESIGN-r4 已由 D4 整体确认）

以下给出已确认决定的具体常量、序列与观察字段。标识可在 to-spec 中等义整理，但不得改变字段含义、优先级、允许/禁止动作或验证边界。
...
to-spec 来源：本文件 ROUTING-DESIGN-r5（仅登记 r4 的整体批准 D4）、R01–R08、Q1–Q16、F01–F07、CE-01–CE-14、ADR-0036/0037、CONTEXT 相关词汇；同一绝对入口 `/Users/nineofour/Agent-Alfred-issue-26-design/tmp/agent-work/issue-26/LOOP.md` 本次交接 revision 9，接续时读取最新 revision。本票工作树/branch/base见首页。

批准源 D1=Q1–Q5、D2=Q6–Q12、D3=Q13–Q16，用户均回复“全按建议”；D4“确认”及被确认文件哈希已记录于首页，批准源 r4 字节已冻结。新会话无需读取聊天历史即可接续；to-spec 必须保留 case身份/具体预期/接缝/外部替换/排除项，不把已确认的上游有限修订抹掉，也不宣布产品已经验证。发布 GitHub、代码实现、提交/推送/合并/关票仍需各自授权。
