# #25 实现：Graph 引擎（builder / compile / wave 执行 / describe）

来源：https://github.com/nineofoursyrup/Agent-Alfred/issues/25
...
## Comment https://github.com/nineofoursyrup/Agent-Alfred/issues/25#issuecomment-5653901873

<!-- issue-25-delivery-graph-r1-c3 -->
#25 已完成实施与验收，PR #60 已合并至 main。

- 候选：`graph-r1-c3`；规范 `GRAPH-SPEC-r1`，SHA256 `bcdd8bf2da29acc7b70e8735902db46afcc0f6e7a98cf1f4a8101f586e3e3fd6`。
- 提交：`5caad8f3bff7e4ac24154d8eeeef07813927b900`；合并：`5d16e21d876eb67994bb3a3b5d6c121689a10652`。远端 main 已回读为该合并 SHA，合并树与原32路径候选提交树完全一致，无额外产品文件。
- Standards PASS：STD-01/02/03 全部由原独立评审者复验关闭，0阻塞、0剩余建议。
- Spec PASS：SPEC-01/02 由原独立评审者复验关闭，0阻塞；SPEC-03（普通边声明重排hash规范化）保持 optional/deferred。

本地门禁：Graph77 + 受影响Host28 = 105 passed；Python **4188 passed / 1 deselected**；浏览器 **177 passed**；Ruff、skills、env、typecheck、build及wheel/sdist × base/mcp四种安装全部通过。四节点工厂真实串联验收3模型请求/3Step/2计量行/1业务行通过。

| CE | 已验证公共行为 |
|---|---|
| CE-01 | 同波普通错误恢复与未接管失败；非法写集撤销整波 |
| CE-02 | NoAction 保留恢复事实、不造空助手回复 |
| CE-03 | Router 只见声明源候选；required 缺席先失败 |
| CE-04 | 非法拓扑、读写来源、跳过支配与工具白名单编译拒绝 |
| CE-05 | 深冻结、输入不污染、None与缺席区分、漏写消费前失败 |
| CE-06 | 两模型波撤销仍保留真实Attempt/预算，Step标aborted_by_wave |
| CE-07 | 全副作用种类白名单与真实账本；none/occurred/unknown精确聚合 |
| CE-08 | 真实删除/文件/MCP未知/迟到/计量失败遵守强制收尾 |
| CE-09 | 固定时钟/ID确定事件、OR fan-in、终点数、describe/hash |
| CE-10 | 异常/未知/缺席路由使整波撤销，不转error边 |
| CE-11 | error专属性编译拒绝；两份合法恢复独立上下文并汇合 |
| CE-12 | 终点出边/空图拒绝、实际output缺席恢复、显式None合法 |
| CE-13 | 未接管失败停止未开始节点；波末非法写集如实保留先前动作 |
| CE-14 | 精确input所有权、required/optional、编译冻结、Registry与invoke隔离 |
| CE-15 | 四工厂共享预算、SDK retry单lease、零Step工具身份、第二图拒绝 |
| CE-16 | 真实RunRecorder/SQLite唯一正式消息、有效fallback、集合/bytes/map输出与原始费用 |

CI与用户批准范围：用户明确要求“linux相关都不进行测试”，随后授权“没问题就收尾到issue closed/completed”。因此 **PR Linux CI = NOT RUN（用户明确豁免）**，**合并后 Linux CI = NOT RUN（用户明确豁免）**。两个提交含 `[skip ci]`；PR head及合并SHA的Actions runs/check-runs均回读为0。这里不把空列表、跳过或未运行写成CI PASS。已执行验收均为macOS本地，真实临时SQLite/文件/MCP子进程；模型与外部服务仅采用规范允许的替身。

后续边界：Host 用户选图UI、两个业务workflow、付费模型与私人服务不在本票范围，不宣称已交付。唯一实施说明：[docs/implementation/issue-25-graph.md](https://github.com/nineofoursyrup/Agent-Alfred/blob/5d16e21d876eb67994bb3a3b5d6c121689a10652/docs/implementation/issue-25-graph.md)；完整合同：[GRAPH-SPEC-r1](https://github.com/nineofoursyrup/Agent-Alfred/blob/5d16e21d876eb67994bb3a3b5d6c121689a10652/docs/design/issue-25-graph-spec.md)。

本地详细证据：实施树 `tmp/agent-work/issue-25/` 下 candidate-c3、checks-c3.json、ce-evidence.json、standards-review/report-c3.md、spec-review/report-c3.md、delivery/；唯一状态入口仍为设计树的 `tmp/agent-work/issue-25/LOOP.md`。本验收已满足用户批准范围，按 completed 原因关闭本票。
