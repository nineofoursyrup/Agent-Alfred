...
来源：https://github.com/nineofoursyrup/Agent-Alfred/issues/21

> **当前规范与验收**：有效合同为下方已确认的 **MCP-SPEC-r2 + D6** 修订与原 r1 保留条款；实施、独立双轴与适用双阶段 CI 已通过。Linux 独立专项验收为 **用户豁免 / NOT RUN**。原 r1 的阶段说明是历史快照；当前逐项结果见文末“实施与验收结论”。
...
本规范为 **MCP-SPEC-r1**，由已整体确认的设计 r5 等义整理；Q1–Q20、IC-01 与 CE-01–CE-15 不变更。正文为完整实施/验收合同，不依赖本地聊天或未发布附件才能解释关键行为。
...
规定明确修订“mcp extra 零依赖”：stdlib 仍拥有 MCP 传输/会话，不引官方 MCP SDK；mcp extra 仅引入 JSON Schema 校验库。只支持协商版本 2025-11-25，schema 方言只接受默认或显式 2020-12。声明必须通过 metaschema 检查；inputSchema/outputSchema 为 JSON object，根 type 为 object，不接受 boolean 根 Schema。inputSchema 原样给模型，arguments 仍只做可序列化 object/资源边界检查后透传服务端，不运行本地业务 schema 校验。
...
首版支持文档内可解析的无环 JSON Pointer $ref，拒绝外部引用、循环引用、$dynamicRef 与需动态作用域/自定义 vocabulary 的声明；不自动重写不支持 schema。允许普通 $defs/$id 作为声明内容，URI 作为标识不自动触发检索；精确引用基址处理应遵循库的资源语义，不能用字符串前缀冒充解析。拒绝 unsupported 工具不影响同服务器的已验证工具，目录显示总数/可用数及原因。此范围是明确受限的 2020-12 工具支持，不声称完整 MCP/JSON Schema 兼容。
...
- Sequence: tools/list 含无效 schema、不支持方言、远程/文件引用、内部引用、循环引用或高代价正则；对照缺失可选依赖的启动。
...
发布后本 GitHub Issue 正文的 **MCP-SPEC-r1** 是实施权威；本地逐字镜像为 `/Users/nineofour/Agent-Alfred-issue-21-design/docs/design/issue-21-mcp-spec.md`。验收定义完整内嵌，设计稿是有 hash 的历史确认来源，不是第二份可变验收合同。
...
<!-- issue21-approved-amendments-r2-d6 -->
## 当前有效修订：MCP-SPEC-r2 + D6

本段记录用户已批准的实施修订，并覆盖前文 r1 的对应条款和旧阶段状态。原 r1 正文保留为发布历史；其余20项决定、IC-01与15个CE的行为合同保持。当前规范身份为 r1 + D5（下列r2替换条款）+ D6；不把旧“NOT STARTED/NOT RUN”阶段快照当作当前验收状态。

**D5**：固定 Everything 包实测声明 Draft-07；用户对“是否将规范扩展为同时支持 Draft-07，再修改实现并重跑验收？”回复“可以”。以下 Q8/Q15 与 CE-08 是已批准 MCP-SPEC-r2 的对应条款，替代 r1 中相同编号；声明保持原文，不修改第三方包。
...
上述条款中的 NOT RUN 是冻结规范时的历史阶段标签；最终行为验收结果见下方实施验收摘要。冻结r2源码：[issue-21-mcp-spec.md](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/docs/design/issue-21-mcp-spec.md)，SHA256 `8fbaf0631d8d37a234d46c8e85960c443c40fcce93e789566957eda73a047f0c`。

**D6**：用户原文“linux本issue忽略，不做测试。”本 Issue 豁免 Linux 实测及 Linux CI 结果作为验收门槛，Linux 状态是 **用户豁免 / NOT RUN**，不能表述为专项测试通过。豁免不删除 Linux 支持、不修改 CI 以补验；macOS、Everything及其他20Q/IC-01/15CE要求不变。既有仓库自动 CI 正常执行，其原始结果单独如实记录，不将其重新包装为额外 Linux 专项验收。

D6独立附件 SHA256 `ebf9859c878a21a8f3712c8c1b2cbd52e137ce15d6f370c2090ef28fc21f1257`；当前完整复合评审身份 `mcp-c4-5c4d5f618889+r2+D6-7ff689881171`，复合 SHA256 `7ff6898811711b460963256ad56fe0bdb521dad441347d0e0be87ea8f1f424ae`。两轴独立评审均PASS，无未解决阻塞或可选建议；SPEC-06的Everything分项实测通过，Linux分项按D6用户豁免收敛。旧r2范围下的Spec INCOMPLETE报告保留原文，新结论不追溯改写旧报告。


<!-- issue21-acceptance-issue21-delivery-20260913-01 -->
## 实施与验收结论

实现 commit `e93934b05121d989d10190b45c0d5381b79287cd`，PR [#59](https://github.com/nineofoursyrup/Agent-Alfred/pull/59)。精确提交已审36文件，commit tree `1c96f884c739d23c225b3166252353862079ef0d` 与 c4 产品内容和文件模式全部匹配，临时证据未混入。

桥接已接通配置→真实stdio发现→Host/Registry/Loop→持久授权→SQLite/Ops→关闭回收。启动enabled与工具allowed分离，未授权不发送业务请求；调用后不确定结果保守unknown、隔离并停止后续调用；费用缺报保持unknown。Schema使用可终止进程，并按原Draft-07/2020-12语义校验，不重写声明/参数，不检索外部资源。

### 独立评审与本地门禁

- 当前复合候选 `mcp-c4-5c4d5f618889+r2+D6-7ff689881171`；Standards **PASS**，Spec **PASS**，blocker=0，optional=0。
- STD-01–03、SPEC-01–05/07–09保持独立resolved；SPEC-06 / Everything实测PASS，SPEC-06 / Linux用户D6豁免、NOT RUN。旧报告原文保留，未把旧INCOMPLETE改写为旧范围PASS。
- macOS全量：**4111 passed, 1 deselected**（默认排除需key用例）；浏览器 **177 passed**；MCP专项 **154 passed**。双方言修复：旧实现6 failed/20 passed → 新实现26 passed；Standards独立37项、Spec独立69项及4个真实语义探针通过。
- wheel/sdist × base/mcp四种干净安装通过，包内源码与c4逐文件哈希匹配；Ruff、TypeScript、skills、env和diff门禁通过。
- 本票独立Linux专项验收：**用户豁免 / NOT RUN**。下列GitHub Actions是既有自动CI的真实结果，非另行安排的Linux专项验收，也不改变D6。

### 固定真实 Everything 互操作

`@modelcontextprotocol/server-everything@2026.8.31`，SDK `1.30.0`，Node `25.8.2`，npm `11.19.0`；完整锁定107项依赖，安装后逐项版本核对一致。入口和SDK未改写，原始echo schema仍声明Draft-07；serverInfo.version=2.0.0仅作为握手信息，不替代实际包版本。

- lock SHA256 `26c7b08b7f7517f6224c36ef9d228fd7cb8b41e2970609d4b7e875fc617182e6`；入口 SHA256 `26eb2809baf531c3a54300e3af15bfdceb7b4de50e785ab8f2f4acf02cbce1fd`。
- 真实发现13/13可用；首次调用账为 `not_executed / not_authorized / not_started`；仅显式允许echo后第二Run `succeeded`，其他12工具保持unset。
- 实际成功写入stdio的所有frames中恰好1次 `tools/call`，参数为 `{"name":"echo","arguments":{"message":"alfred-mcp-smoke"}}`；真实返回文本 `Echo: alfred-mcp-smoke`，模型收到结果，成本unknown。不是只验证callable入口或替换协议结果。
- SQLite请求账与DashboardApi→Host→SQLite的Ops detail一致；真实PID59962的进程组消失、子进程reap、三条管道关闭、reader退出、Session.closed及Host.close均true。第三方服务没有为D6复核或发布重跑。

### CE-01–CE-15逐项证据

除已声明的ScriptedModel及外部MCP协议夹具外，Host/Registry/Loop/SQLite/HTTP、校验器子进程和资源生命周期均用真实路径。排序敏感案例使用FIFO/条件屏障/明确OS故障注入。以下入口固定到实际实现commit，不会随main前移漂移。

| CE | 已验证行为 | 结果与固定提交测试入口 |
|---|---|---|
| CE-01 | 启动与调用授权分离；真实未授权零发送 | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L25) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L1051) |
| CE-02 | 替换/重启身份对账、权限历史与路由 | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L241) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L1086) |
| CE-03 | 发送后超时、取消/迟到/重复；隔离后新Run隐藏 | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L251) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L445) |
| CE-04 | 完整目录、空目录、多服务器与坏分页隔离 | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L721) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L780) |
| CE-05 | 双向请求拒绝、安全文本/非文本、stderr及浏览器XSS | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L445) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L948) |
| CE-06 | 正常/异常/SIGINT/SIGTERM/SIGKILL与旧资源归属 | macOS实测通过；Linux D6用户豁免/NOT RUN；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L765) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L1355) |
| CE-07 | 真实零字节/部分写入，not_sent与unknown分账 | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L630) |
| CE-08 | Draft-07/2020-12语义与原文；引用/方言/vocabulary限制，网络/文件零检索 | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_dialects.py#L24) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L290) |
| CE-09 | 严格配置、恢复、stale token/同id/忙拒绝、HTTP交错 | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L181) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L531) |
| CE-10 | allowed退役/denied保留、合同/密钥/目录变化、持久失败 | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L318) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L368) |
| CE-11 | 成功/错误/RPC矩阵、双方言输出校验、IC-01与费用unknown | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L84) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L598) |
| CE-12 | 数量/字节/depth边界、真实alias碰撞、分页与30秒发现时限 | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L387) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L721) |
| CE-13 | 实际validator SIGSTOP/kill/reap，未回收拒绝替代 | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L614) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp.py#L695) |
| CE-14 | env一致发布/回滚/暂停、单服重连、迟到页面响应 | 已验证；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L214) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L561) |
| CE-15 | 清理失败/继续清理、Host.close重试、操作容量/过期及归属 | macOS实测通过；Linux D6用户豁免/NOT RUN；[测试1](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L492) · [测试2](https://github.com/nineofoursyrup/Agent-Alfred/blob/e93934b05121d989d10190b45c0d5381b79287cd/src/agent_alfred/evals/deterministic/test_mcp_review.py#L531) |


相关IC-01、补充反例和6个MCP浏览器案例见提交中的 `docs/implementation/issue-21-mcp.md`、`test_mcp_review.py`、`test_mcp_dialects.py` 与 `tests/browser/mcp.spec.js`。不以计数或fixture通过替代真实Everything，也不扩大到相邻票、Windows、任意Schema全兼容或SIGKILL后无条件回收。
...
| 阶段 | 精确身份 | 结果 |
|---|---|---|
| 提交/PR | commit `e93934b05121d989d10190b45c0d5381b79287cd`；[PR #59](https://github.com/nineofoursyrup/Agent-Alfred/pull/59) | 精确36文件、正常推送，MERGED |
| PR CI | head `e93934b05121d989d10190b45c0d5381b79287cd`；[run 34751000666](https://github.com/nineofoursyrup/Agent-Alfred/actions/runs/34751000666) | completed / success；所有适用步骤通过 |
| 合并 | SHA `7f38e8b6abe77022315584e0fd9d098d9219c470`；parents `35a053e5e793257187aea0a66a93fbc8de252dd0` + `e93934b05121d989d10190b45c0d5381b79287cd` | 实际merge tree `1c96f884c739d23c225b3166252353862079ef0d`与已审候选一致；main包含 |
| 合并后CI | head `7f38e8b6abe77022315584e0fd9d098d9219c470`；[run 34751595158](https://github.com/nineofoursyrup/Agent-Alfred/actions/runs/34751595158) | push / completed / success；所有适用步骤通过 |

合并使用预期head约束，前后回读base/head/check/merge SHA。上述自动CI遵循仓库既有流程；未为了D6新增Linux专项测试或改动CI。PR使用Refs #21，未通过合并提前自动关闭本票。收尾授权原文：“执行收尾直到issue closed/completed”。规范与本摘要回读一致后，才以completed原因显式关闭。未强推、删分支/工作树、发版本或部署。


## Comment https://github.com/nineofoursyrup/Agent-Alfred/issues/21#issuecomment-5425984031

📌 前置调研 #9 已关闭（`docs/research/mcp-bridge.md`，601 行）。硬结论：

1. **不用官方 `mcp` SDK，自写约 150 行纯 stdlib 客户端。** 实测 `uv pip install mcp` = 28 包 / 28 MB / 1008 文件，13 个必需依赖全部无条件安装、**无 extra 开关**；其中 `cryptography` 13 MB（OAuth 拖进来的）+ starlette/uvicorn/sse-starlette/httpx2（**服务端** HTTP 栈）+ opentelemetry-api，**约 21 MB 与 stdio 客户端毫无关系**。调研已写出 110 行 stdlib 版本并跑通官方 `@modelcontextprotocol/server-everything` v2.0.0（退出码 0）。
2. **MCP 已分裂成两个世代**——`2026-07-28` modern 取消了 `initialize` 握手，改为每请求 `_meta` 携带版本与能力，协议定义为无状态；`2025-11-25` 及更早是 legacy。生态仍在 legacy（官方 SDK 默认发 2025-11-25）。**v1 只做 legacy**，消息层给 `_meta` 留空位。
3. **命名空间化的真实约束不是 MCP 规范**：规范说 1–128 字符且允许点号，但 **Anthropic Messages API 与 OpenAI 都是 `^[a-zA-Z0-9_-]{1,64}$`**，规范允许的点号在两家都会 400。所以 `server_tool` 的分隔符只能用 `_` 或 `-`，且**总长必须截到 64**。规范另外明确 `serverInfo.name` **不保证唯一**，不该拿来做前缀。
4. **子进程清理必须 `start_new_session=True` + `os.killpg()`**：MCP 服务器几乎都被 `npx` / `uv run` 拉起，`proc.terminate()` 只杀 wrapper。pgid 用 `proc.pid` 而非 `os.getpgid()`（组长被回收后后者会失败）。实测顽固服务器（忽略 SIGTERM + 带孙子进程）走完信号梯度 2.01s 整组消失。`atexit` 不可靠（三种情况不跑），需四层防线，第四层要求 `stdin=PIPE` 且持唯一写端。
5. **四态映射的四条易错规则**：`isError: true` **不**降级连接状态；stderr 有输出 **≠** 出错（规范原文 SHOULD NOT assume）；状态必须**观测得出**而非配置推断；错误必须带可操作原因 + **脱敏后的** stderr 尾部。

✅ **新增验收项**（调研顺手挖到的）：离线测试加 `-W error::ResourceWarning`，等于零成本的进程泄漏检测器。


## Comment https://github.com/nineofoursyrup/Agent-Alfred/issues/21#issuecomment-5427881281

来自 [#4](https://github.com/nineofoursyrup/Agent-Alfred/issues/4) 的裁决要点：

- **MCP 入参透传给服务端校验**，不在本地跑 JSON Schema；服务端的 `isError: true` 映射为 `execution_error`。
- **运行中服务器失联：保留 Schema，调用返回 `unavailable`，不动态移除工具**——同一个 Run 内能力集合漂移比一个诚实的错误更难解释。`unavailable` 与 `execution_error` 必须分开（换工具 vs 改参数重试）。
- Registry **构造后不可变**：握手在 Agent 构造期完成，变更走「重建 Agent」这一档 `reload_scope`。
- alias 生成走调研 §5.3 的确定性算法，**改名必须经 `notice` + Tools 页显式报出**；**调用只查映射表，绝不解析名字**。
- 工具 `description` 带「来自 MCP 服务器 `<server_key>`」来源标注。
- 中止走 `notifications/cancelled`（ADR-0006：真正的中止归工具 IO 层）。

## Comment https://github.com/nineofoursyrup/Agent-Alfred/issues/21#issuecomment-5652736990

<!-- issue21-closeout-issue21-delivery-20260913-01 -->
已完成MCP桥接实施与验收；当前合同为正文已同步的MCP-SPEC-r2 + D6。

- [PR #59](https://github.com/nineofoursyrup/Agent-Alfred/pull/59)已合并，commit `e93934b05121d989d10190b45c0d5381b79287cd`，merge `7f38e8b6abe77022315584e0fd9d098d9219c470`；提交和合并树均匹配已审36文件候选。
- [PR CI 34751000666](https://github.com/nineofoursyrup/Agent-Alfred/actions/runs/34751000666)与[合并SHA CI 34751595158](https://github.com/nineofoursyrup/Agent-Alfred/actions/runs/34751595158)均completed/success，分别绑定实际head与merge SHA。
- Standards PASS，Spec PASS，0 blockers/0 optional；CE-01～15逐项测试入口、IC-01、固定Everything真实echo与资源回收证据已写入正文。
- 本地4111 passed/1 deselected、浏览器177、四制品安装及静态检查通过；Everything13/13可用、未授权零执行、仅1次允许echo成功且成本unknown。
- Linux独立专项验收为用户D6明确豁免/NOT RUN，不把豁免写成测试通过；既有自动CI结果按原始记录报告。

正文已完整回读，保留r1历史并明确r2/D6覆盖范围。按用户“执行收尾直到issue closed/completed”授权，接下来以completed原因关闭本票。
