...
来源：https://github.com/nineofoursyrup/Agent-Alfred/issues/26
...
## Comment https://github.com/nineofoursyrup/Agent-Alfred/issues/26#issuecomment-5662264771

<!-- issue26-routing-c4-closeout -->
# #26 消息分流交付验收

实现提交：`486fa7a846f9f76ddcedc3c5573e3ae2f7a20b5f`；PR：#62；合并提交：`53a9041cfb22f85fd4f3bc10c264cda4c4179fe3`。

验收身份：`routing-c4-86938cebf894` / `ROUTING-SPEC-r1`。46 文件的提交树与已审/已测候选逐字节及 mode 一致。规范 SHA256：`9db162b731625af3f6cd129dd811ae9ca2e8c3ff596d9de9c6bca351685d69e5`；manifest SHA256：`86938cebf894b9e960eed864b91279342eb31aa1deb7a77e5287fdb0f0c36f18`。

## 验收矩阵

| 要求 | 已交付与验证 |
|---|---|
| R01 | 默认关闭的持久 Behaviour 开关，CLI/Web 共享；旧/新基线精确对照及损坏配置备份恢复。 |
| R02 | 固定 DAG、闭合分类标签与代码保护；未知分类进入显式 fallback，模型不能选择 node_id。 |
| R03 | quick/full/NoAction 三条路径及独立无回复原因；无空助手记录或虚假任务保存。 |
| R04 | 真实上下文投影与专属恢复；CompletedWithRecovery/NoAction 保留恢复事实，来源不可信时安全停止。 |
| R05 | 按副作用状态、取消、总截止和剩余预算控制单次回退；持久保留原图错误及最终停止原因。 |
| R06 | 一次 Skill 快照、共享 Step/deadline、辅助模型分类输入隔离与真实 Attempt 计量。 |
| R07 | SQLite/HTTP/SSE/CLI/浏览器持久投影；刷新、重启、trace裁剪、记录失败和NoAction窗口均验证。 |
| R08 | 真实 Graph/Host/Registry/SQLite/MCP stdio/HTTP/SSE/browser 离线验证；只在批准的外部边界替换。 |

CE-01–CE-14 全部通过独立证据核验：关闭基线、分类保护、NoAction与真实任务、故障回退、上下文来源、动作不重复、设置捕获/繁忙拒绝、输入与转录隔离、记录/重启/CLI、预算和截止、上下文不变量、NoAction窗口与提炼、配置IO故障、MCP工具与图同代发布。

独立评审：Standards **PASS**（独立28项）；Spec **PASS**（独立25项）；STD-01/02、SPEC-01/02/03 均 resolved，0 未决阻塞、0 可选建议、0 待决合同。SPEC-03 原错误被覆盖的反例已先复现失败，再由默认回归及真实 SQLite 探针验证修复。

## 检查与交付证据

- 本地 Python：4390 passed / 1 deselected；浏览器：185 passed。
- 本地 ruff/typecheck/Skill与环境检查/构建/wheel、sdist × base、MCP四组安装：PASS。
- [PR head CI](https://github.com/nineofoursyrup/Agent-Alfred/actions/runs/34828531956)：SUCCESS；4390 Python / 185 browser 通过，绑定 `486fa7a846f9f76ddcedc3c5573e3ae2f7a20b5f`。
- [合并后 CI](https://github.com/nineofoursyrup/Agent-Alfred/actions/runs/34829791573)：SUCCESS；4390 Python / 185 browser 通过，绑定 `53a9041cfb22f85fd4f3bc10c264cda4c4179fe3`。
- 合并树与已审候选树一致；目标 main 已包含合并提交。

本地证据索引（未混入产品提交）：实现树 `tmp/agent-work/issue-26/verification-c4.md`、`candidate-c4/`、`acceptance-c4.json`、`standards-review-c4.md`、`spec-review-c4.md` 和 `delivery/`；唯一状态入口为设计树 `tmp/agent-work/issue-26/LOOP.md`。

付费模型测试 **NOT RUN**；离线结果不证明真实模型分类语义质量。本票不包含 #27 手动聚合、通用流程编辑器或输入准备重排，相邻票保持原范围。用户已明确授权“执行全部收尾工作”；双阶段CI和验收回读完成后以 completed 关闭本票。
